...
var EntityStore = Class({
    ...
    update: function (type, id, rev, data, opts, done) {
        opts || (opts = {});
        this.engine.update(type, id, rev, data, function (err, result) {
            if (err && err.code == 'NONEXIST' && opts.refresh) {
                this.engine.fetch(type, [id], {}, function (fetchErr, results) {
                    if (fetchErr) {
                        done(fetchErr);
                    } else if (results.length == 0) {
                        done(Errors.nonexist(id));
                    } else {
                        var result = normalizeQueryResult(results[0]);
                        done(Errors.conflict(id, { rev: rev, entity: result }), result);
                    }
                });
            } else {
